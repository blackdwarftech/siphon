...
class MongoDBMemoryStore(MemoryStore):
    ...
    def __init__(self, url: str) -> None:
        try:
            ...
            raise RuntimeError("motor and certifi are required for MongoDB memory storage") from exc
    ...
    async def close(self) -> None:
        """Close the MongoDB client and release connections."""
        self.client.close()
    ...
    async def get(self, phone_number: str) -> Optional[CallerMemory]:
        ...
        try:
            ...
            if doc:
                ...
                logger.info(f"Loaded memory from MongoDB for {_redact_phone(phone_number)}: {memory.total_calls} calls, {len(memory.summaries)} summaries")
                return memory
            logger.debug(f"No memory found in MongoDB for {_redact_phone(phone_number)}")
            return None
        except Exception as e:
            logger.error(f"MongoDB get failed for {_redact_phone(phone_number)}: {e}")
            return None
    ...
    async def save(self, phone_number: str, memory: CallerMemory) -> None:
        ...
        try:
            ...
            logger.info(f"Saved memory to MongoDB for {_redact_phone(phone_number)}: {memory.total_calls} calls, {len(memory.summaries)} summaries")
        except Exception as e:
            logger.error(f"MongoDB save failed for {_redact_phone(phone_number)}: {e}")
    ...
    async def delete(self, phone_number: str) -> bool:
        ...
        try:
            ...
            logger.error(f"MongoDB delete failed for {_redact_phone(phone_number)}: {e}")
            return False
    ...
    async def exists(self, phone_number: str) -> bool:
        ...
        try:
            ...
            logger.error(f"MongoDB exists check failed for {_redact_phone(phone_number)}: {e}")
            return False
